...
export default function GuardianConsoleClient() {
  ...
  const copy =
    locale === "pt"
      ? {
          ...
          trafficTitle: "Pulso de tráfego",
          trafficDescription: "Separado dos observers internos para mostrar sinais reais de acesso ao Pi.",
      ...
      : {
          ...
          trafficTitle: "Traffic pulse",
          trafficDescription: "Kept separate from internal observers so the panel shows real access signals to Pi.",
    ...
    <div className="min-h-screen bg-[#050505] p-8 text-white">
      <header className="mb-12 flex flex-col gap-6 border-b border-white/5 pb-8 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2 rounded-full border border-red-500/20 bg-red-500/10 px-4 py-2 text-xs font-bold text-red-300">
            <Lock size={14} />
            {copy.role}: {role}
          </div>
        </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <section className="lg:col-span-2 rounded-[32px] border border-white/5 bg-white/[0.02] p-8">
          <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="text-right">
              <Kicker color="faint">{copy.source}</Kicker>
            ...
            {snapshot?.guardian.guardian.checks.map((check) => (
              ...
              >
                <div>
                  ...
                  <Kicker color="faint" className="mt-2">{check.id}</Kicker>
                </div>
        ...
        <aside className="space-y-6">
          ...
          <div className="rounded-[32px] border border-white/5 bg-gradient-to-br from-red-900/20 to-slate-900/20 p-6">
          ...
          <div className="rounded-[32px] border border-white/5 bg-white/[0.02] p-6">
            ...
            <p className="mb-6 text-sm leading-relaxed text-gray-500">{copy.trafficDescription}</p>

            <div className="space-y-3">
              <div className="rounded-2xl border border-white/5 bg-black/30 px-4 py-3">
                <p className="text-[10px] font-black uppercase tracking-[0.24em] text-gray-600">External 24h</p>
                <p className="mt-2 text-2xl font-black text-white">{snapshot?.overview.traffic?.external.last24h ?? 0}</p>
              </div>
              <div className="rounded-2xl border border-white/5 bg-black/30 px-4 py-3">
                <p className="text-[10px] font-black uppercase tracking-[0.24em] text-gray-600">Unique 24h</p>
                <p className="mt-2 text-2xl font-black text-white">{snapshot?.overview.traffic?.uniqueClients.last24h ?? 0}</p>
              </div>
              <div className="rounded-2xl border border-white/5 bg-black/30 px-4 py-3">
                <p className="text-[10px] font-black uppercase tracking-[0.24em] text-gray-600">{copy.lastSeen}</p>
                <p className="mt-2 text-sm font-mono text-gray-200">{trafficLastSeen}</p>
              </div>
            </div>
          </div>
        </aside>
      ...
      <section className="mt-8 rounded-[32px] border border-white/5 bg-white/[0.02] p-8">
        <div className="mb-8">
          <h2 className="flex items-center gap-3 text-sm font-black uppercase tracking-[0.24em] text-white">
            <Network className="text-red-500" size={18} />
