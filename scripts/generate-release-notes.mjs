...
export function generateLatestReleaseNotes({ output, count = 3 } = {}) {
  const source = path.resolve(repoRoot, 'internal-docs/project-publication-log.md');
  const target = path.resolve(repoRoot, output || 'internal-docs/latest-release-notes.md');
  ...
  const lines = [
    ...
    '',
    'This file is generated from `internal-docs/project-publication-log.md` and summarizes the latest project publication records.',
    '',
    `- Generated at: ${now}`,
    `- Repository: ${remote}`,
    `- Branch: ${branch}`,
